    ...
    <div class="animate-fade-in flex flex-col mx-auto w-full md:max-w-4xl h-[92vh] md:h-auto space-y-6 pt-2 px-4">
        ...
        <div class="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6 md:p-10 space-y-8">
            ...
            <div class="flex flex-col items-center gap-4">
                <button v-if="userRole === 'admin'" @click="salvarNoBanco" 
                        class="btn-primary w-full md:w-auto md:min-w-[220px] py-4 text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-100">
                    Salvar Alterações
                </button>
        ...
        <div class="bg-slate-900 rounded-[1.5rem] p-5 flex items-center gap-4 border border-slate-800 shrink-0">
            ...
            <p class="text-[10px] text-slate-400 font-medium leading-relaxed text-left">
                Dica: Alterar estes valores atualizará o cálculo do <b class="text-white">Preço Sugerido</b> para novos itens e o <b class="text-white">Lucro Líquido</b> em todo o sistema.
            </p>
        </div>
    </div>`,
    ...
    async salvarNoBanco() {
      ...
      try {
        await window.supabase
          .from("configuracoes")
          .update({ valor: this.localTaxas.ml_comissao })
          .eq("chave", "ml_comissao");
        await window.supabase
          .from("configuracoes")
