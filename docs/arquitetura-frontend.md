...
**Deploy**: Azure Static Web Apps (recurso `happy-rock-0fa653d10`, criado em 2026-07). Workflow gerado automaticamente pelo Azure ao conectar o recurso ao GitHub (`.github/workflows/azure-static-web-apps-happy-rock-0fa653d10.yml`), editado manualmente para adicionar `npm ci` + `npm run build:css` antes do deploy — o preset "Custom" do Azure não sabe rodar nosso script de build customizado sozinho (só reconhece `npm run build`, e o nosso é `build:css`). App location `/`, output location `/` (a build gera o CSS no lugar, não move pra uma pasta separada). **Importante**: existia um `deploy.yml` antigo apontando para uma Azure App Service chamada `marketplacemanagement` — só que esse é o nome do recurso do **backend** (.NET API, ver `master_marketplacemanagement.yml` no repo do backend); rodar aquele workflow teria sobrescrito o deploy da API com os arquivos estáticos do frontend. Removido junto com `nginx.conf` (que era da mesma abordagem abandonada, App Service com container).
