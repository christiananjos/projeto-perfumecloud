...
**Deploy**: Azure Static Web Apps (recurso `happy-rock-0fa653d10`, criado em 2026-07). Workflow gerado automaticamente pelo Azure ao conectar o recurso ao GitHub (`.github/workflows/azure-static-web-apps-happy-rock-0fa653d10.yml`).

- `scripts/build-site.mjs` (rodado via `npm run build`) gera o CSS do Tailwind e monta uma pasta `dist/` com só o que deve ser público (`index.html` + `assets/`, excluindo `.test.js` e `tailwind.src.css`). O workflow aponta `output_location: "dist"` e deixa o **Oryx** (build interno da action `Azure/static-web-apps-deploy@v1`) detectar e rodar `npm run build` sozinho — não há step manual de build no workflow.
- **Achado de segurança (histórico de depuração desta primeira publicação, 3 tentativas até funcionar)**:
  1. Com `output_location: "/"` (config gerada pelo Azure por padrão), a raiz inteira do repositório ficava pública via HTTP — `node_modules/`, `docs/`, `e2e/`, `.claude/`, `README.md`, `package.json` etc. todos acessíveis, só `.git/`/`.github/` excluídos por padrão do Azure.
  2. Um arquivo `.swaignore` foi tentado e **não teve nenhum efeito** — removido.
  3. `output_location: "dist"` foi adicionado junto com `skip_app_build: true` (rodando o build manualmente antes) — o deploy "teve sucesso" no log, mas a exposição **continuou** (confirmado testando o site ao vivo depois do deploy). Causa: com `skip_app_build: true`, a action parece ignorar `output_location` e empacotar `app_location` inteiro mesmo assim — `output_location` só é respeitado quando o Oryx de fato executa a etapa de build.
  4. Correção final: removido `skip_app_build`, deixando o Oryx rodar `npm run build` (que já existe no `package.json` desde a tentativa anterior) por conta própria — assim ele empacota corretamente só `output_location: dist`. `tailwindcss` também precisou ser movido de `devDependencies` para `dependencies`, já que o `npm install` do Oryx pode rodar com `NODE_ENV=production` (que pula `devDependencies`) e o build depende dele.
  5. Validado no site ao vivo: `package.json`/`README.md`/`docs/`/`e2e/`/`.claude/` agora dão 404; a aplicação carrega e estiliza normalmente.

  Nenhum segredo real chegou a ser exposto durante a depuração (nenhum arquivo do repo contém credencial em texto puro), mas era exposição desnecessária de código e configuração interna.

**Importante**: existia um `deploy.yml` antigo apontando para uma Azure App Service chamada `marketplacemanagement` — só que esse é o nome do recurso do **backend** (.NET API, ver `master_marketplacemanagement.yml` no repo do backend); rodar aquele workflow teria sobrescrito o deploy da API com os arquivos estáticos do frontend. Removido junto com `nginx.conf` (que era da mesma abordagem abandonada, App Service com container).
...
- Gerado: `assets/css/tailwind.css` (minificado, via `npm run build:css`, chamado por `scripts/build-site.mjs` dentro de `npm run build`) — **não commitado** (`.gitignore`), gerado no CI a cada deploy.
