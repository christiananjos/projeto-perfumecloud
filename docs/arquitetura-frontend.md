...
**Deploy**: Azure Static Web Apps (recurso `happy-rock-0fa653d10`, criado em 2026-07). Workflow gerado automaticamente pelo Azure ao conectar o recurso ao GitHub (`.github/workflows/azure-static-web-apps-happy-rock-0fa653d10.yml`), editado manualmente:

- `npm ci` + `npm run build` (script `build`, não `build:css`) antes do step de deploy, com `skip_app_build: true` — o preset "Custom" do Azure só reconhece `npm run build` sozinho (nosso build de CSS é `build:css`), e mesmo se reconhecesse, empacotaria a raiz inteira do repo.
- `npm run build` (`scripts/build-site.mjs`) gera o CSS do Tailwind **e monta uma pasta `dist/`** com só o que deve ser público (`index.html` + `assets/`, excluindo `.test.js` e `tailwind.src.css`). `output_location: "dist"` no workflow — **achado de segurança da primeira tentativa de deploy**: com `output_location: "/"`, o Azure publicava a raiz inteira do repositório (`node_modules/`, `docs/`, `e2e/`, `.claude/`, `README.md`, `package.json` etc. todos ficavam acessíveis publicamente via HTTP, só `.git/`/`.github/` eram excluídos por padrão). Um arquivo `.swaignore` foi tentado primeiro e **não teve nenhum efeito** (não é respeitado quando `skip_app_build: true` pula a etapa de build/empacotamento do Oryx) — removido depois que a curadoria via `dist/` resolveu o problema de verdade. Nenhum segredo real chegou a ser exposto (nenhum arquivo do repo contém credencial em texto puro), mas era exposição desnecessária de código/config interna.
